...
router.get('/', (req, res) => {
    ...
    Bloc.find((err, docs) => {
        if (!err) {
            anass = JSON.parse(JSON.stringify(docs));
            console.log("anass"+ anass);
            for (var i = 0; i < anass.length; i++) {
                var counter = anass[i];
...
router.get('/planing/:id/:name', (req, res) => {
    ...
    Occupation.find((err, docss) => {
        anass = JSON.parse(JSON.stringify(docss));
            console.log("anass"+ anass);
            ...
            for (var i = 0; i < anass.length; i++) {
                ...
                if(counter.salle==req.params.id){
                    if(youssef.indexOf(counter.date)>-1){
                        khaoula.push(counter);
                    }
                }
                }
                console.log("khaouka "+ khaoula);
        if (!err) {
